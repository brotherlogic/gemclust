...
- **Declarative Infrastructure & Dependencies:** A local Grafana instance operates via a `HelmRelease` resource inside `infrastructure/grafana/`. The definition includes built-in node tolerations to safely handle Master/Control Plane limitations on single-node development clusters.
